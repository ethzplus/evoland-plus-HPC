...
if (is.null(params$data$dem)) {
  stop("params$data$dem is not set")
}
if (is.null(params$data$prodreg)) {
  stop("params$data$prodreg is not set")
}
...
# Create new column with region + elevation class
# in the attribute table of regelev
...
clip_and_reclassify <- function(lulc, out) {
  ...
  lulc_r <- terra::classify(lulc, m1)

  # Creating the rasters for each region
  list_reg_elev <- data.frame(unique(regelev$regelev_n))
  colnames(list_reg_elev) <- "regelev"

  # Iterates through each unique combination of production region and
  # elevation class.
  for (i in seq_len(nrow(list_reg_elev))) {
