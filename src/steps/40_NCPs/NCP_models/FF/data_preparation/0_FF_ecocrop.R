...
ecocrop_models <- function(crops, ta, pr, ph, out_dir) {
...
for (year in years) {
  for (rcp in rcps) {
    ...
    print(paste("Year:", year, "RCP:", rcp, "Temperature files:", length(ta_year_rcp), "Precipitation files:", length(pr_year_rcp)))
    ...
    ecocrop_models(crops, ta_year_rcp, pr_year_rcp, ph, current_out_dir)
  }
}
