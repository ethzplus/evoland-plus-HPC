...
inspect_indexed_png <- function(png_path) {
  ...
  cat(paste(
    "  Color Type:",
    color_types[as.character(info_attr$color.type)],
    "(",
...
prepare_lulc_files <- function(
  ...
  if (!dir.exists(file.path(base_dir, perc_area_data_dir))) {
    dir.create(
      file.path(base_dir, perc_area_data_dir),
      recursive = TRUE,
      showWarnings = FALSE
    )
  }
  if (!dir.exists(file.path(base_dir, area_chg_data_dir))) {
    dir.create(
      file.path(base_dir, area_chg_data_dir),
      recursive = TRUE,
      showWarnings = FALSE
    )
  }
  ...
  future_sapply(unique(lulc_df$Scenario_ID), function(Scenario) {
    ...
    scenario_LULC_dat[
      which(
        scenario_LULC_dat$NOAS04_2018 == Glacier_val &
          !(scenario_LULC_dat$ID %in% Glacier_IDs)
      ),
      "NOAS04_2018"
    ] <- unlist(Aggregation_scheme[
    ...
    for (i in 1:nrow(scenario_lulc_df)) {
      ...
      cat(paste(
        ...
        scenario_lulc_df$Config_ID[i],
        "at time step:",
        scenario_lulc_df$Time_step[i],
        "\n"
      ))
        ...
        if (length(map_masks) > 0) {
          ...
          for (mask_name in mask_names) {
            ...
            if (file.exists(mask_path)) {
              # get the mask paths
              mask_path_tif <- scenario_lulc_df[
                i,
                paste0("lulc_path_tif_", mask_name)
              ]
              map_path_png <- scenario_lulc_df[
                i,
                paste0("lulc_path_png_", mask_name)
              ]
              mask_path_data <- scenario_lulc_df[
                i,
                paste0("lulc_path_perc_area_", mask_name)
              ]
              ...
              json_data <- toJSON(
                setNames(as.list(rast_tbl$class_name), rast_tbl$perc_area),
                pretty = TRUE
              )
          ...
          cat(paste(
            "Saved raster layer to:",
            scenario_lulc_df$lulc_path_tif[i],
            "\n"
          ))
          ...
          cat(paste(
            "Saved table of LULC % areas to:",
            scenario_lulc_df$lulc_path_perc_area[i],
            "\n"
          ))
          ...
          cat(paste(
            "Saved map image to:",
            scenario_lulc_df$lulc_path_png[i],
            "\n"
          ))
        }
      } else {
    ...
    for (Config_ID in unique(scenario_lulc_df$Config_ID)) {
      ...
      config_time_steps <- config_time_steps[
        config_time_steps != first_time_step
      ]
        ...
        for (mask_name in mask_names) {
          ...
          area_change_path <- file.path(
            ...
            paste0(
              ...
              "-",
              config_lulc_df$Econ_scenario[
                config_lulc_df$Time_step == time_step
              ],
              "-",
              config_lulc_df$Pop_scenario[
                config_lulc_df$Time_step == time_step
              ],
              "-",
          ...
          if (
            !paste0("lulc_path_perc_area_", mask_name) %in%
              colnames(config_lulc_df)
          ) {
            stop(paste(
              "No column for lulc_path_perc_area_",
              mask_name,
              "in config_lulc_df"
            ))
          }
          ...
          first_data <- data.frame(
            class_name = unlist(first_data),
            perc_area = as.numeric(names(first_data)),
          ...
          current_data <- data.frame(
            ...
            perc_area = as.numeric(names(current_data)),
            stringsAsFactors = FALSE,
            row.names = NULL
          )
          ...
          # convert the df to json
          area_change_json <- toJSON(
            setNames(
              as.list(area_change$class_name),
